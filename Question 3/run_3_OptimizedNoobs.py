...
if __name__ == '__main__': 
	...
	opt = TBD_obj.fit(X_train, y_train)

	# End the timer
	execTime = (datetime.now() - t0).total_seconds()

	# Check the error on the training set 
	trErr = TBD_obj.error(TBD_obj.pred(X_train), y_train)

	# Check the error on the test set
	teErr = TBD_obj.error(TBD_obj.pred(X_test), y_test)

	# Get full info about the statistics of the network 
	TBD_obj.last_train_stat_full()

	print("Time for optimizing the network: \'{} seconds".format(round(execTime,4))+'\'')
	print(f"Training Error: \'{round(trErr, 5)}\'")
	print(f"Test Error: \'{round(teErr, 5)}\'")
	TBD_obj.plotApproximationFunc()
	TBD_obj.save_optimized_params('TBD_params')
	TBD_obj.save_hyperparams('TBD_hyperParam')
