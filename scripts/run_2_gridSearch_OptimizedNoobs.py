...
############################
'''
list_C = [0.0001, 0.001, 0.001, 0.1]
list_q = [4, 6]
list_gamma = [ 0.005, 0.01, 0.1, 1, 5]

# Param grid 
params_grid = [(gamma, C, q) for gamma in list_gamma for C in list_C for q in list_q]

# Keys: params
# Values: (training accuracy, test accuracy)
param_score = dict()
...
for params in params_grid: 
    ...
    # Build a model using the parameters
    model = SVM(kernel_function = poly_kernel, C = C, gamma = gamma)
    model.SVM_decomposition(X_train, y_train, q = q)
    
    # Predict the y_train 
    y_train_pred = model.predict(X_train)
    train_accuracy = model.accuracy(y_train_pred, y_train)
    
    # Predict the y_test
    y_test_pred = model.predict(X_test)
    test_accuracy = model.accuracy(y_test_pred, y_test)
    
    # Store the accuracy of the model on the trainig set and the test set for the model 
    param_score[params] = (train_accuracy, test_accuracy)

# Sort the score of the params based on the test accuray 
Sorted_param_score = {k: v for k, v in sorted(param_score.items(), key=lambda item: item[1])}

# Take the model that had the best test accuracy 
gamma, C, q = list(Sorted_param_score)[-1]
